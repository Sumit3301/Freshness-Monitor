#!/usr/bin/env python3
"""
Chicken Freshness Data Preparation Pipeline (3-Stage)
=====================================================
Extracts color features specifically for chicken freshness classification
from the chicken runs in Final Runs/ directory.

Usage:
    python prepare_chicken_data.py
"""
...
def compute_color_stats(image: np.ndarray) -> dict:
    """Compute perceptual features: HSV, RGB, CIE L*a*b*, and ratio indices."""
...
def extract_features(image_path: str) -> dict:
    """Extract color features from a single chicken film image."""
...
def prepare_chicken_dataset():
    """Discover chicken training images, extract features, and save to chicken_features_3stage.csv."""
    print("=" * 60)
    print("🐔 Chicken Freshness Classification — Data Preparation (3-Stage)")
    print("=" * 60)

    aug_dir = os.path.join(config.BASE_DIR, "augmented_chicken")
    os.makedirs(aug_dir, exist_ok=True)
    os.makedirs(config.MODEL_DIR, exist_ok=True)

    chicken_images = []
    for run_folder in CHICKEN_RUN_FOLDERS:
        folder_path = os.path.join(FINAL_RUNS_DIR, run_folder)
        ...
        for filename in sorted(os.listdir(folder_path)):
            ...
            chicken_images.append((filepath, label, source))

    print(f"\n🖼️ Found {len(chicken_images)} chicken training images across {len(CHICKEN_RUN_FOLDERS)} runs.")

    all_features = []
    for filepath, label, source in chicken_images:
        ...
        for i, aug_img in enumerate(aug_images):
            ...
            aug_num["label"] = label
            aug_num["source"] = f"augmented_chicken/{aug_name}"
    ...
    if all_features:
        ...
        print(f"\n✅ Saved {len(all_features)} chicken samples to {csv_path}")
