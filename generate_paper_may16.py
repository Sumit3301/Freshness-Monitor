"""
Research Paper Generator for run_16-may-2026 (merged 16+17 May, 0h-36h, 37 images)
Same structure as the original Research_Paper.docx
"""
import os, sys
import cv2
import numpy as np
...
# ─── CONFIG ────────────────────────────────────────────────────────────────────
RUN_DIR   = "run_16-may-2026"
RUN_DATE  = "16 May 2026"
...
fig.tight_layout()
c_sat = save_chart(fig, "m01_saturation_decay")
...
fig.tight_layout()
c_rgb = save_chart(fig, "m02_rgb_progression")
...
fig.tight_layout()
c_rb = save_chart(fig, "m03_rb_gap")
...
fig.tight_layout()
c_lab = save_chart(fig, "m04_lab_channels")
...
fig.tight_layout()
c_hsv = save_chart(fig, "m05_hsv_all")
...
fig.tight_layout()
c_dist = save_chart(fig, "m06_stage_distribution")
...
fig.tight_layout()
c_model = save_chart(fig, "m07_model_performance")
print("  Chart 7/9 done")

# 8. Dominant Colors Heatmap
apply_style()
# sample every ~3h to keep chart readable across 36h
...
fig.tight_layout()
c_dc = save_chart(fig, "m08_dominant_colors")
...
c_dash = save_chart(fig, "m09_dashboard")
...
sub = doc.add_paragraph(f"Experimental Run Report  |  Run: {RUN_DIR}  |  Date: {RUN_DATE}  |  Span: {start_h}h – {end_h}h  |  Captures: {total_images}")
...
doc.add_paragraph(
    ...
    f"experimental run {RUN_DIR} (merged from 16 May and 17 May 2026 captures), which spans {end_h} hours "
    f"({total_images} captures), providing a complete per-hour colorimetric dataset. "
    "The system achieves 95.15% cross-validated accuracy using only commodity hardware (Raspberry Pi 3B+, ~USD 35) "
...
body("Pi continuously captures at 15 fps (640x480); raw JPEG bytes pushed to POST /frame; JS-polled MJPEG at /stream.", bold_prefix="Mode 1 - Real-Time Video Streaming: ")
...
doc.add_paragraph(
    ...
    f"This run covers the full spoilage arc from fresh (0h) through advanced spoilage ({end_h}h)."
)
...
for k, v in [
    ...
    ("Source runs merged", "run_16-may-2026 (0h-14h) + run_17-may-2026 (0h-21h)"),
...
for row in [
    ...
    ("run_20-april-2026",    "Apr 2026", "19",  "114", "Mixed",  "Improved"),
...
bullet("Multi-day run merging: Seamless hour-continuity merging across calendar days for extended monitoring.")

# 7. CONCLUSION
h1("7. Conclusion")
doc.add_paragraph(
    f"This report documents experimental run {RUN_DIR} ({RUN_DATE}), a merged dataset comprising {total_images} "
    f"hourly captures spanning {start_h}h to {end_h}h (combining 16 May and 17 May 2026 captures). "
    "The colorimetric analysis confirms consistent film color evolution across all four freshness stages, "
    "with HSV Saturation and b* providing the strongest discriminative signals. "
